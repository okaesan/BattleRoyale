		...
		if (sender instanceof Player) {
			...
			case "set":
				if(args.length == 1){
					...
					if(ClubManager.isClub(args[2])){
						...
						player.sendMessage(ChatColor.GREEN + "プレイヤー <" + args[1] + "> に部活 <" + ClubManager.getClub(args[2]).getDisplayName() + "> を設定しました。");
						return true;
					}
					else{
			...
			case "remove":
				if(args.length == 1){
					...
					if(ClubManager.hasClub(Bukkit.getPlayerExact(args[1]))){
						...
						player.sendMessage(ChatColor.GREEN + "プレイヤー <" + args[1] + "> から部活 <" + c + "> を削除しました。");
						return true;
					}
					else{
			...
			case "get":
				if(args.length == 1){
					...
					if(ClubManager.hasClub(Bukkit.getPlayerExact(args[1]))){
						...
						player.sendMessage("プレイヤー " + ChatColor.LIGHT_PURPLE + "<" + Bukkit.getPlayerExact(args[1]).getName() + ">" + ChatColor.RESET + " の部活");
						player.sendMessage(ChatColor.AQUA + "<" + ClubManager.getClub(Bukkit.getPlayerExact(args[1])).getName() + "> : <" + ClubManager.getClub(Bukkit.getPlayerExact(args[1])).getDisplayName() + ">");
						return true;
					}
					else{
						player.sendMessage(ChatColor.RED + "部活は設定されていません。");
			...
			case "list":
				...
				for(Club c : ClubManager.getClubList()){
					player.sendMessage(ChatColor.AQUA + "<" + c.getName() + "> : <" + c.getDisplayName() + ">");
				}
				return true;
			}
		}
