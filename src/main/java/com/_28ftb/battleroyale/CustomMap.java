...
public class CustomMap extends MapRenderer {
  ...
  public static List<Integer> deathRandom = PlusThreadClass.deathNotRandom
      .get(PlusThreadClass.rootRandom.get(StartCommand.playCount));
  ...
  public void render(MapView map, MapCanvas canvas, Player player) {
    ...
    /*
     * 猶予エリアの描画 追加されたときのみ描画する。
     */
    for (int i : PlusThreadClass.deathRandomCount) {
      if (i >= PlusThreadClass.deathNotRandom
          .get(PlusThreadClass.rootRandom.get(StartCommand.playCount)).size()) {
        break;
      }
      ...
      if ((int) locL.get(0) >= (int) locR.get(0) && (int) locL.get(1) >= (int) locR.get(1)) {
        for (int x = 0; x < locPerPix * 4; x++) {
          for (int z = 0; z < locPerPix * 4; z++) {
            canvas.setPixel((int) ((pdaX - edgeX) / pixPerLoc) - x,
                (int) ((pdaZ - edgeZ) / pixPerLoc) - z, (byte) 122);
          }
        }
    ...
    for (int i : PlusThreadClass.deathRandomCountPast) {
      ...
      if ((int) locL.get(0) >= (int) locR.get(0) && (int) locL.get(1) >= (int) locR.get(1)) {
        for (int x = 0; x < locPerPix * 4; x++) {
          for (int z = 0; z < locPerPix * 4; z++) {
            canvas.setPixel((int) ((pdaX - edgeX) / pixPerLoc) - x,
                (int) ((pdaZ - edgeZ) / pixPerLoc) - z, (byte) 18);
          }
        }
      } else if ((int) locL.get(0) < (int) locR.get(0) && (int) locL.get(1) < (int) locR.get(1)) {
        for (int x = 0; x < locPerPix * 4; x++) {
