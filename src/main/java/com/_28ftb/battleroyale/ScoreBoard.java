...
public class ScoreBoard {
  ...
  public static void scoreSide(boolean nowPlay) {
    ...
    // int maxPersons =
    // teamBoard.getTeam(TEAM_ALIVE_NAME).getPlayers().size()+teamBoard.getTeam(TEAM_DEAD_NAME).getPlayers().size();
    ...
    // スコアボードの中身
    o.getScore("" + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + "禁止エリア追加まで " + ChatColor.RESET + ": "
        + PlusThreadClass.loopC).setScore(2);
    if (0 < countDown.attackCountDown) {
      o.getScore("" + ChatColor.RED + ChatColor.BOLD + "攻撃可能になるまで" + ChatColor.RESET + ": "
          + countDown.attackCountDown).setScore(1);
    } else {
    ...
    o.getScoreboard()
        .resetScores("" + ChatColor.YELLOW + "残り人数 " + ChatColor.RESET + ": " + playerSize);
    playerSize = teamBoard.getTeam(TEAM_ALIVE_NAME).getPlayers().size();

    o.getScore("" + ChatColor.YELLOW + "残り人数 " + ChatColor.RESET + ": "
        + teamBoard.getTeam(TEAM_ALIVE_NAME).getPlayers().size()).setScore(-1);

    o.getScoreboard().resetScores("" + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + "禁止エリア追加まで "
        + ChatColor.RESET + ": " + String.valueOf(PlusThreadClass.loopC + 1));
    o.getScoreboard().resetScores(
        "" + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + "禁止エリア追加まで " + ChatColor.RESET + ": 0");
    o.getScoreboard().resetScores("" + ChatColor.RED + ChatColor.BOLD + "攻撃可能になるまで"
        + ChatColor.RESET + ": " + String.valueOf(countDown.attackCountDown + 1));
