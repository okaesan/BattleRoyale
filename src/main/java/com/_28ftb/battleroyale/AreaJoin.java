...
public class AreaJoin {
  ...
  public static void joinArea() {
    ...
    for (Player player : Bukkit.getServer().getOnlinePlayers()) {
      ...
      if (betaLx >= playerLocationX && playerLocationX >= betaRx && betaLz >= playerLocationZ
          && playerLocationZ >= betaRz) {
        // Lがx座標、z座標共にRより大きい場合
        JoinSystem.onJoin(player);
      } else if (betaLx < playerLocationX && playerLocationX < betaRx && betaLz < playerLocationZ
          && playerLocationZ < betaRz) {
        // Lがx座標、z座標共にRより小さい場合
        JoinSystem.onJoin(player);
      } else if (betaLx >= playerLocationX && playerLocationX >= betaRx && betaLz < playerLocationZ
